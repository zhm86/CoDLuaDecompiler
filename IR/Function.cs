    ...
    public class Function
    {
        ...
        public void ConstructControlFlowGraph()
        {
            ...
            for (int i = 0; i < Instructions.Count(); i++)
            {
                ...
                else if (Instructions[i] is Return ret2)
                {
            ...
            foreach (var node in PostorderTraversal(true))
            {
                ...
                if (node.LoopFollow != null && node.LoopFollow != node && node.Predecessors.Count() >= 2 && node.LoopType == CFG.LoopType.LoopPretested)
                {
                    ...
                    if (node.Instructions.Last() is Jump loopJump && loopJump.Condition is BinOp loopCondition && loopCondition.Operation == BinOp.OperationType.OpLoopCompare && node.Instructions[^2] is Assignment aaa3 && aaa3.Right != null && aaa3.Right is BinOp)
                    {
                        ...
                        if (loopInitializer.Instructions.Count > 2 && loopInitializer.Instructions[loopInitializer.Instructions.Count - 2] is Assignment initassn)
                        {
